...
- Evidence before reporting back: the one decisive check (tests/typecheck, or render smoke for UI) with exit code/output. Once per code state — after a fix, run it again, but never repeat the same check without an intervening change, and no extra verification rounds beyond that one piece of evidence.
...
1. `VERIFICATION:` check run + result (exit code / test count / output excerpt)
2. `DEVIATIONS:` deliberately left out, deviations from the handoff, risks (or "none")
